...
=cut
    ...
    $ArgList{ODM_SUPPORT}                = $ENV{MTK_ODM_SUPPORT};
...
sub ProcessRawPartitionLayoutData
{
	...
	if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "efuse")
	{
        ...
        {
            if ($ArgList{ODM_SUPPORT} ne "yes" || $ArgList{MTK_AB_OTA_UPDATER} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        ...
        {
            if ($ArgList{ODM_SUPPORT} ne "yes" || $ArgList{MTK_AB_OTA_UPDATER} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
	...
	if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^(cache|recovery)$/i)
	{
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "vbmeta")
        {
            if ($ArgList{MTK_BOARD_AVB_ENABLE} ne "true" || $ArgList{MTK_AB_OTA_UPDATER} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^vbmeta_[ab]$/i)
        {
            if ($ArgList{MTK_BOARD_AVB_ENABLE} ne "true" || $ArgList{MTK_AB_OTA_UPDATER} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
...
sub GenPartSizeFile_iniFile
{
    ...
    foreach my $part (@partition_layout_cooked)
    {
        ...
        {    
            $temp = $part->{Size_KB} * 1024;
            if ($part_name eq "dtbo" || $part_name eq "odmdtbo")
            {
...
sub GenPGPTFile
{
   ...
   {
       my $part = $partition_layout_cooked[$piter];
       if ((exists $SkipPartList{$part->{Partition_Name}}) == 0)#skip partitions not in gpt table
       {
