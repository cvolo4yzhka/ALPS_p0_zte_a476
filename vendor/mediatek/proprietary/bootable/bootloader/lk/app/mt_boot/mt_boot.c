...
static int create_gz_dt_node(void *fdt)
{
	...
	PAL_ASSERT(kernel_addr != 0);

	reg_property[0] = cpu_to_fdt32(0);
	reg_property[1] = cpu_to_fdt32(kernel_addr);
		   ...
		   void *ramdisk, unsigned ramdisk_sz)
{
	...
	if (hw_mdtbo_index != 0xff) {
		snprintf(tmpbuf, TMPBUF_SIZE, "androidboot.dtbo_idx=%d", hw_mdtbo_index);
		cmdline_append(tmpbuf);
	}
	...
	if (!has_set_p2u) {
		...
		if (logo_lk_t != 0) {
			pal_log_err("[PROFILE] 1st logo takes %d ms\n", logo_lk_t);
